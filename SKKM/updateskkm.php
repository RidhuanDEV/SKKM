...
<body class="bg-no-repeat bg-cover" style="background-image: url(background2.avif)">
...
if (isset($_POST['submit'])) {
    ...
    // $querycek = "SELECT nrp FROM datamahasiswa WHERE nrp = $nrp";
    // $eksekusiquerycek = $koneksi ->query($querycek);
    // $rowcek = $eksekusiquerycek->fetch_assoc();
    ...
    if (!empty($errors)) {
            echo "<script type='text/javascript'>alert('Mohon isi semua persyaratan dengan benar !');</script>";
    }else{
    // Update database with file path
    $querySQL = "INSERT INTO penyimpanan (`nrp`,`id`,`kegiatan`,`satuan`,`tingkat`,`dokumen`,`NilaiSKKM`) VALUES ('$nrp','$id','$kegiatan','$Satuan','$tingkat','$nama_dokumen','$nilaiSKKM')";
    ...
    <div class ='p-10 block w-1/2 min-h-screen border bg-transparent rounded-xl '>
        ...
        <div class='flex items-center justify-center w-full'>
            <label for='dokumen' class='flex flex-col items-center justify-center w-full h-56 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600'>
                <div class='flex flex-col items-center justify-center pt-5 pb-6'>
                    ...
                    <img src='filesvg.svg' alt='' id='filesvg' class='hidden'>
        ...
        <div class = 'p-2 my-8 w-24 h-10 items-center justify-center flex border-2 border-white active:bg-green-300 bg-red-400 shadow-2xl rounded-md text-center font-bold hover:underline hover:border-black hover:cursor-pointer'>
            <a href='halamansiswa.php'>Kembali</a>
        </div>
        </form>
    </div>";
    }
    // dari sini dap lu ubah sesuai di pdf skkm yg project 2
    if ($id == 2){
        echo"
    <div class ='p-10 block w-1/2 min-h-screen border bg-transparent rounded-xl '>
        <form id='myForm' method=".'post'." enctype=".'multipart/form-data'.">
