...
<body class="font-sans bg-gray-100 text-gray-900 tracking-wider leading-normal">
    ...
    <main class="overflow-x-hidden h-full flex">
        <div class="w-1/4 h-screen"></div>
        <nav class="w-1/4 bg-slate-900 p-8 flex fixed h-full flex-col gap-4 text-white justify-center items-center">
            <img src="../assets/admin.png" alt="FOTO" width="144px" height="144px">
            <?php 
                ...
                ";
            ?>
            <button onclick="activeClassHandler('dashboard')" id="dashboardButton" class='bg-gray-700 rounded-xl font-mono active:scale-105 px-20 py-2 border border-white'>Dashboard</button>
            <button onclick="activeClassHandler('DataPengajuan')" id="DataPengajuanButton" class='bg-gray-700 rounded-xl font-mono active:scale-105 px-10 py-2 '>Data Pengajuan SKKM</button>
        ...
        <section id="DataPengajuan" class="opacity-0 w-0 h-0 mx-auto  overflow-auto">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <tr class='text-center'>
                                ...
                                <td>" . $row["dokumen"] . "</td>
                                <td>" . $row["NilaiSKKM"] . "</td>
                                <td><a href='../crud/acceptskkm.php?noreg=" . $row["noreg"] . "&id=".$row["id"]."' onclick='return confirmUpdateSKKM()' class='text-green-500 hover:underline'>Setujui</a></td>
                                <td><a href='../crud/acceptskkm.php?noreg=" . $row["noreg"] . "' onclick='return confirmTolakSKKM()' class='text-red-500 hover:underline'>Tolak</a></td>
                                </tr>";
                            }
                        }
        ...
        <section class="opacity-100 transition-all duration-500 w-3/4 h-screen self-start flex-1 px-2" id="dashboard">
            ...
            <div class="w-full h-full flex gap-8">
                <div class="w-1/2 h-1/4 hover:scale-[99%]">
                    <div class="w-full h-full bg-green-400 border-b-8 border-l-4 border-green-600 rounded-xl flex justify-center items-center">
                        <button class="w-full h-full overflow-hidden" onclick="activeClassHandler('data')">
                        <div class="flex justify-center items-center">
                            <img src="../assets/user.png" alt="User" class="hover:scale-[95%]">
                        </div>
                        <div class="pt-8 hover:scale-[105%]">
                        <?php 
                ...
                <div class="w-full h-1/4 hover:scale-[99%]">
                    <div class="w-1/2 h-full bg-blue-500 border-b-8 border-l-4 border-blue-600 rounded-xl flex justify-center items-center">
                        <button class="w-full h-full overflow-hidden" onclick="activeClassHandler('DataPengajuan')">
                        <div class="flex justify-center items-center">
                            <img src="../assets/icons8-file-64.png" alt="User" class="hover:scale-[95%] decoration-white">
                        </div>
                        <div class="pt-8 hover:scale-[105%]">
                        <?php 
                        $usertotal = "SELECT count(nrp) as total FROM penyimpanan" ;
                        $Eksekusi = $koneksi->query($usertotal);
                        $row = mysqli_fetch_assoc($Eksekusi);
                        ?>
                            <p class="font-mono font-bold text-lg text-slate-800">Total Pengajuan SKKM : <?php echo $row['total']; ?></p>
                        </div>
                    </button>
                    </div>
                </div>
        ...
        <section id="data" class="opacity-0 transition-all duration-500 w-0 h-0 self-start mx-auto px-2 overflow-auto">
            ...
            <h1 class="flex items-center font-sans font-bold break-normal text-slate-900 text-xl md:text-2xl">
                ...
                <?php
                    ...
                    <div>
                    <a href='../crud/create.php' onclick='return confirm('Are you sure you want to create data?')'
                    class='text-white hover:underline border-b-4 border-l-2 bg-blue-400 border-blue-500 active:scale-95 p-2 rounded-md'>Create Data Mahasiswa</a>
                    </div>
                    </div>
                    " : "");
                    ?>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                <tr class='text-center  '>
                                    ...
                                    <td>" . $keterangan . "</td>
                                    <td>
                                        <a href='../mahasiswa/profilmahasiswa.php?nrp=" . $row["nrp"] . "' class='text-blue-500 hover:underline'>Read</a>    
                                    </td>
                                    ".( $_SESSION['user_id']['role'] == 'admin' ? "
                                    <td>
                                        <a href='../crud/updateprofil.php?nrp=".$row["nrp"]." onclick='return confirmUpdate()' class='text-green-500 hover:underline'>Update</a>
                                    </td>
    ...
    function activeClassHandler(target) {
        ...
        if (target == "DataPengajuan") {
            ...
            dashboardElement.classList.add("opacity-0", "w-0","h-0")
            dashboardElement.classList.remove("opacity-100", "w-3/4", "px-2","h-screen","flex-1")
        } else if (target == "data") {
            ...
            dashboardElement.classList.add("opacity-100", "w-3/4", "px-2","h-screen","flex-1")
            dashboardElement.classList.remove("opacity-0", "w-0","h-0")
            dataElement.classList.add("opacity-0", "w-0","h-0")
            dataElement.classList.remove("opacity-100", "w-3/4", "px-2","h-full","flex-1")
            DataPengajuanElement.classList.add("opacity-0", "w-0","h-0")
            DataPengajuanElement.classList.remove("opacity-100", "w-3/4", "px-2","h-full","flex-1")
